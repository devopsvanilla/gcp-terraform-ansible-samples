...
resource "hpe_morpheus_cypher_secret" "vm_nginx_tfvars" {
  ...
  value = jsonencode({
    poc_name                         = var.poc_name != null && var.poc_name != "" ? var.poc_name : "vm-nginx-terraform-ansible"
    ...
    manage_vm_external_ip_org_policy = var.manage_vm_external_ip_org_policy != null ? var.manage_vm_external_ip_org_policy : true
    ...
    run_ansible                      = var.run_ansible != null ? var.run_ansible : true
    ansible_wait_seconds             = var.ansible_wait_seconds != null ? var.ansible_wait_seconds : 15
    ansible_max_retries              = var.ansible_max_retries != null ? var.ansible_max_retries : 3
    ansible_private_key_file         = var.ansible_private_key_file != null && var.ansible_private_key_file != "" ? var.ansible_private_key_file : "<%=cypher.read('secret/ansible-private-key')%>"
    ansible_ssh_user                 = var.ansible_ssh_user != null && var.ansible_ssh_user != "" ? var.ansible_ssh_user : "devopsvanilla_ansible"

    vms = {
      (var.vm_name != null && var.vm_name != "" ? var.vm_name : "vm-nginx-poc") = {
        vm_name                          = var.vm_name != null && var.vm_name != "" ? var.vm_name : "vm-nginx-poc"
