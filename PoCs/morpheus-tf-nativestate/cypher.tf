...
# Os parâmetros do formulário (Option Types) são injetados dinamicamente pelo Morpheus no Terraform em tempo de execução.
resource "hpe_morpheus_cypher_secret" "vm_nginx_tfvars" {
  ...
  value = jsonencode({
    ...
    name                  = "<%= customOptions.vm_name ?: app?.name ?: 'vm-gcp-poc' %>"
    vm_name               = "<%= customOptions.vm_name ?: app?.name ?: 'vm-gcp-poc' %>"
